...
class Generator:
    def __init__(self, column_types=None, context_len=2, language='ru'):
        """
        Конструктор
    ...
    def fit_transform(self, data, path):
        """
        Fit OneHotEncoder-а и генерация признаков (на этих данных будем тренировать классификатор)
        ...
        self._multi_encoder = ColumnApplier(dict([(i, preprocessing.LabelEncoder()) for i in range(len(features_list[0]))]))
        sparse_features_list = self._multi_encoder.fit(features_list, None).transform(features_list)
    ...
    def transform(self, data, path):
        """
        Генерация признаков на обученом OneHotEncoder-e
        :param data: Список слов для извлечения признаков.
